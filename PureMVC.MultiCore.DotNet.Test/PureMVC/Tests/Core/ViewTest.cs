...
namespace PureMVC.Tests.Core
{
	...
	public class ViewTest : BaseTest
    {
		...
		public void RegisterAndNotifyObserver()
        {
            ...
            INotification notification = ViewTestNote.Create(name, 10);
  		 ...
  		 * invoked by the View.
  		 */
  		private readonly IDictionary<string, int> viewTestVars = new Dictionary<string, int>();
		...
		public void HasMediator()
		{
			...
			var mediator = new Mediator(name, this);
			view.RegisterMediator(mediator);
			
   			// assert that the view.hasMediator method returns true
   			// for that mediator name
			Assert.IsTrue(view.HasMediator(name), "Expecting view.hasMediator(name) == true");
		...
		public void MultiThreadedOperations()
		{
			...
			{
			    var t = new Thread(MultiThreadedTestFunction) {Name = "ControllerTest" + i};
			    threads.Add(t);
			}
