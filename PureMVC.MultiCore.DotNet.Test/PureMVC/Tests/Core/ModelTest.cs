...
namespace PureMVC.Tests.Core
{
	...
	public class ModelTest : BaseTest
    {
		...
		public void RegisterAndRemoveProxy()
        {
			...
			model.RegisterProxy(new Proxy(name, new List<int>(new[] { 7, 13, 21 })));
		...
		public void HasProxy()
		{
			...
			IProxy proxy = new Proxy(name, new List<string>(new[] { "clubs", "spades", "hearts", "diamonds" }));
			model.RegisterProxy(proxy);
			
   			// assert that the model.hasProxy method returns true
   			// for that proxy name
			Assert.IsTrue(model.HasProxy(name), "Expecting model.hasProxy(name) == true");
